...
pub struct FrameworkState {
    pub wake_registry: Arc<Mutex<WakeRegistry>>,
    pub arena: UiArena,
    ...
    pub scroll_offsets: HashMap<NodeId, Vec2>,
}
...
impl FrameworkState {
    ...
    pub fn update_aabbs(
        ...
        if !needs_rebuild {
            if self.arena.taffy_map.len() != self.node_bounds.len() {
                ...
                if !needs_rebuild {
                    self.is_overlay_pass = true;
                    for &overlay in &self.arena.overlays.clone() {
                        let mut op_pos = Vec2::ZERO;
                        if let Some(ob) = self.node_bounds.get(&overlay) {
                            op_pos = Vec2::new(ob[0], ob[1]);
                        }
                        if self.check_layout_changed(engine, overlay, op_pos, true) {
                            needs_rebuild = true;
                            break;
                        }
                    }
        ...
        // Ekran hajmini aniqlaymiz
        let root_bounds = self
            .node_bounds
            .get(&root)
            .copied()
            .unwrap_or([0.0, 0.0, 1024.0, 768.0]);
        self.screen_size = Vec2::new(root_bounds[2], root_bounds[3]);

        // ====================================================================
        // INQILOB: Aydin Portallar va Z-Index Mantiqi
        // ====================================================================
        self.is_overlay_pass = true;
        for &overlay_node in &self.arena.overlays.clone() {
            let mut start_pos = Vec2::ZERO;
            if let Some(target_id) = self.arena.anchors.get(&overlay_node) {
                if let Some(target_node_id) = self.arena.dynamic_nodes.get(target_id) {
                    if let Some(t_node) = self.arena.taffy_map.get(target_node_id) {
                        if let Some(bounds) = self.node_bounds.get(t_node).copied() {
                            let mut oh = 0.0;
                            let mut ow = 0.0;
                            if let Ok(children) = engine.taffy.children(overlay_node) {
                                if let Some(&child) = children.first() {
                                    if let Ok(layout) = engine.taffy.layout(child) {
                                        oh = layout.size.height;
                                        ow = layout.size.width;
                                    }
                                }
                            }

                            let mut px = bounds[0];
                            let mut py = bounds[1] + bounds[3];

                            // COLLISION DETECTION: Sig'masa joyini o'zgartirish
                            if py + oh > self.screen_size.y && bounds[1] - oh >= 0.0 {
                                py = bounds[1] - oh;
                            }
                            if px + ow > self.screen_size.x {
                                px = (self.screen_size.x - ow).max(0.0);
                            }

                            start_pos = Vec2::new(px, py);
                        }
                    }
                }
            }

            // Click-Outside uchun ko'rinmas qutini (Backdrop) joylashtiramiz
            if let Some(&overlay_id) = self.arena.node_map.get(&overlay_node) {
            ...
            // Endi Overlay'ning o'zini va farzandlarini rekursiv joylashtiramiz (Ularning z-index'i yuqoriroq bo'ladi!)
            self.build_aabb_recursive(engine, overlay_node, start_pos, initial_clip);
        }
        self.is_overlay_pass = false;
    }
    ...
    fn build_aabb_recursive(
        ...
        // Z-Index har qadamda oshib boradi
        self.current_z_index += 1;
        if let Some(&node_id) = self.arena.node_map.get(&node) {
            self.spatial_grid.insert(
                node_id,
        ...
        let mut children_parent_pos = Vec2::new(pos.x, pos.y);

        // =========================================================
        // INQILOB: AGAR SHU ELEMENT SKROLL BO'LGAN BO'LSA,
        // BOLALARINI AVTOMATIK RAVISHDA ORQAGA/TEPAGA SURIB YUBORAMIZ!
        // =========================================================
        if let Some(&node_id) = self.arena.node_map.get(&node) {
            if let Some(scroll) = self.scroll_offsets.get(&node_id) {
                children_parent_pos.x -= scroll.x;
                children_parent_pos.y -= scroll.y;
            }
        }
    ...
    pub fn cycle_focus(&mut self, forward: bool) {
        ...
        for (taffy_node, &node_id) in &self.arena.node_map {
            if let Some(widget) = self.arena.get(node_id) {
                if widget.is_interactive() && widget.type_name() == "TextInput" {
                    interactives.push(*taffy_node);
                }
            }
        }
        ...
        interactives.sort_by(|a, b| {
            let bounds_a = self.node_bounds.get(a).unwrap_or(&[0.0, 0.0, 0.0, 0.0]);
            let bounds_b = self.node_bounds.get(b).unwrap_or(&[0.0, 0.0, 0.0, 0.0]);
            if (bounds_a[1] - bounds_b[1]).abs() > 5.0 {
                bounds_a[1].partial_cmp(&bounds_b[1]).unwrap()
            } else {
                bounds_a[0].partial_cmp(&bounds_b[0]).unwrap()
            }
        });

        let current_idx = self
            .focused_node
            .and_then(|n| interactives.iter().position(|&x| x == n));

        let next_idx = match current_idx {
            Some(idx) => {
                if forward {
                    (idx + 1) % interactives.len()
                } else {
                    (idx + interactives.len() - 1) % interactives.len()
                }
            }
            None => {
                if forward {
        ...
        let new_focus = interactives[next_idx];

        if let Some(old) = old_focus {
            if let Some(&old_id) = self.arena.node_map.get(&old) {
                if !self.sparse_update_queue.contains(&old_id) {
                    self.sparse_update_queue.push(old_id);
                }
            }
        }

        self.focused_node = Some(new_focus);
        if let Some(&new_id) = self.arena.node_map.get(&new_focus) {
            if !self.sparse_update_queue.contains(&new_id) {
                self.sparse_update_queue.push(new_id);
            }
        }
