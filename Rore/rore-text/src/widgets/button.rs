...
// Yadroviy API'larni chaqiramiz
use rore_types::{impl_layout_modifiers, LayoutModifiers};

pub struct Button {
    pub id: String,
    pub style: Prop<Style>, // TO'G'RILANDI: Style o'rniga Prop<Style>
...
// Makroni ulaymiz (Barcha .width(), .expand() API lar shu yerda ulanadi)
impl_layout_modifiers!(Button);

impl Button {
    pub fn new(id: &str) -> Self {
        Self {
            id: id.to_string(),
            style: Prop::Static(Style::default()), // TO'G'RILANDI
    ...
    pub fn style(mut self, style: impl IntoProp<Style>) -> Self {
        self.style = style.into_prop(); // TO'G'RILANDI
        self
    }
...
impl Widget for Button {
    ...
    fn build(
        ...
        // TO'G'RILANDI: Prop ichidan statik qiymatni ajratib olish (clone() xatosi)
    ...
    // rore-core/src/widgets/button.rs faylida 143-qator atrofida:
    ...
    fn handle_event(&mut self, state: &mut FrameworkState, event: &WidgetEvent) -> EventResult {
        if let Some(id) = self.node_id {
            let new_target = match event {
                ...
                // INQILOB: HoverLeave yoki MouseCancel bo'lsa, tugma tinch holatiga (normal) qaytadi
                WidgetEvent::HoverLeave | WidgetEvent::MouseCancel => {
                ...
                // INQILOB: MouseUp faqat rangni tiklaydi, hech qanday mantiq bajarmaydi!
                WidgetEvent::MouseUp => {
                    let c = self.live_hover.lock().unwrap();
                    Some([c.r, c.g, c.b, c.a])
                }
                // Haqiqiy harakat faqat Click kelganda bajariladi
                WidgetEvent::Click => {
    ...
    fn render(
        ...
        let cmd = DisplayCommand::DrawQuad {
            ...
            color_start: self.target_color,
            color_end: self.target_color,
            gradient_angle: 0.0,
