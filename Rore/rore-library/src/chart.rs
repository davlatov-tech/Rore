...
// TO'G'RILANDI: Makro va API'larni chaqirib olamiz
...
pub struct CandlestickChart {
    ...
    pub style: Prop<Style>, // TO'G'RILANDI: Endi bu Prop<Style>
...
// TO'G'RILANDI: Makroni muvaffaqiyatli ulaymiz
impl_layout_modifiers!(CandlestickChart);
...
impl Widget for CandlestickChart {
    ...
    fn build(
        ...
        // TO'G'RILANDI: Statik qiymatni ajratib olish
    ...
    fn handle_event(&mut self, state: &mut FrameworkState, event: &WidgetEvent) -> EventResult {
        // TO'G'RILANDI: "changed" warning'ni yo'q qilish uchun mantiq tozalab yozildi
        match event {
            ...
            WidgetEvent::MouseScroll { delta_y, .. } => {
                self.zoom.update(|z| {
    ...
    fn render(
        ...
        if !data.is_empty() && layout.width > 0.0 && layout.height > 0.0 {
            ...
            let grid_color = Color::hex("#ffffff").with_alpha(0.03);
